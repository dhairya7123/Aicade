...
class GameScene extends Phaser.Scene {
    ...
    update(time, delta) {
    // Add this near the existing enemy spawn code
    ...
    if (this.player.x > this.endPole.x - 20) {
        ...
        if ((this.cursors.left.isDown || this.joystickKeys.left.isDown) && this.player.x > this.cameras.main.scrollX) {
            ...
            this.player.setVelocityX(160);
            this.player.flipX = true;//camera correction
        ...
        if (time > this.nextEnemyTime) {
            this.spawnEnemy();
            this.nextEnemyTime = time + Phaser.Math.Between(3000, 8000);
        }
...
//Enemy spawn rate fixed
    ...
    // Add this new method to the GameScene class to spawn flying enemies
    spawnFlyingEnemy() {
    if (!this.canSpawnEnemies) return;
    
    let x = this.player.x + this.game.config.width / 2;
    let y = Phaser.Math.Between(50, this.game.config.height / 2); // Spawn higher up
    
    let flyingEnemy = this.enemies.create(x, y, 'enemy').setScale(.18);
    flyingEnemy.setTint(0xff0000); // Red tint to distinguish flying enemies
    flyingEnemy.isFlying = true; // Flag to identify flying enemies
    
    // Set initial velocities for diagonal movement
    flyingEnemy.setVelocityX(-200);
    flyingEnemy.setVelocityY(0); // Start with horizontal movement
    
    // Reduce gravity effect for flying enemies
    flyingEnemy.setGravityY(-250); // Counteract some of the world gravity
    
    // Add sine wave movement
    this.tweens.add({
        targets: flyingEnemy,
        y: y + 100, // Amplitude of sine wave
        duration: 2000,
        ease: 'Sine.easeInOut',
        yoyo: true,
        repeat: -1
...
// Modify the collision handler to handle flying enemies differently
onPlayerEnemyCollision(player, enemy) {
    if (enemy.isFlying) {
        // Flying enemies are more dangerous - they always hurt the player unless shot
        if (player.power_state === PLAYER_STATE.BULLETS) {
            ...
            // Flying enemies cause immediate game over unless player has bullet power
